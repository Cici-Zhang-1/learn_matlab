...
mdlFW = fitnlm(x,FW,model,beta0, 'ErrorModel', 'combined','Options', opts);
%disp(mdl)
[yFW,yci] = predict(mdlFW,x1);
yup = yci(:, 2);
ydown = yci(:, 1);
...
mdlFH = fitnlm(x,FH,model,beta0, 'ErrorModel', 'combined','Options', opts);
%disp(mdl)
[yFH,yci] = predict(mdlFH,x1);
yup = yci(:, 2);
ydown = yci(:, 1);
