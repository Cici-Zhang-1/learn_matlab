...
filename_results = 'Structure_MRI_N171-82Q_Two_factors_anvoa_results_male';
no = '';
MW_Combine = [];
MH_Combine = [];
Combine = [];
...
if strcmp(type, 'CC/ExternalCapsule')
    writetable(anovatbl, [folder filename_results '.xlsx'], 'WriteVariableNames', true, ...
        'WriteRowNames', true, 'Sheet', 'CC_ExternalCapsule');
else
    writetable(anovatbl, [folder filename_results '.xlsx'], 'WriteVariableNames', true, ...
        'WriteRowNames', true, 'Sheet', type);
end
