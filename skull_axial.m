...
middleSlice = thirdD/2;
toshow = zeros(firstD, secondD, thirdD);
showHistory = zeros(firstD, secondD, thirdD, 10);
minHistory = 1;
maxHistory = 1;
currentHistory = 1;
% for i = 1:secondD
...
s = uicontrol('Parent',f,'Style','slider','Position',[80,180,740,23],...
              'value',middleSlice, 'min',1, 'max',thirdD, 'SliderStep', [0.0056 0.0056]);
...
b = uicontrol('Parent',f,'Style','pushbutton','Position',[250,140,30,30],...
          ...
          b.Callback = @(src, event) display_roi(s.Value, imgobj, ...
              str2num(e_slice.String), c_prev_next.Value, c_roi_optimize.Value,thirdD, 1);
b_drawassisted = uicontrol('Parent',f,'Style','pushbutton','Position',[400,140,30,30],...
        ...
        b_drawassisted.Callback = @(src, event) display_roi(s.Value, imgobj, ...
                str2num(e_slice.String), c_prev_next.Value, c_roi_optimize.Value,thirdD, 2);
b_drawcircie = uicontrol('Parent',f,'Style','pushbutton','Position',[430,140,30,30],...
        ...
        b_drawcircie.Callback = @(src, event) display_roi(s.Value, imgobj, ...
                str2num(e_slice.String), c_prev_next.Value, c_roi_optimize.Value,thirdD, 3);
b_drawellipse = uicontrol('Parent',f,'Style','pushbutton','Position',[460,140,30,30],...
        ...
        b_drawellipse.Callback = @(src, event) display_roi(s.Value, imgobj, ...
                str2num(e_slice.String), c_prev_next.Value, c_roi_optimize.Value,thirdD, 4);
...
b_auto_pixel = uicontrol('Parent', f, 'Style', 'pushbutton', 'Position', [370, 140, 30, 30], ...
          ...
          b_auto_pixel.Callback = @(src, event) auto_pixel(s, imgobj, e, e_min_piexl, thirdD, c_prev_next.Value);
b_medfilt = uicontrol('Parent', f, 'Style', 'pushbutton', 'Position', [550, 140, 30, 30], ...
              'value', 0, 'min', 0, 'max', 1, 'String', 'M');
          b_medfilt.Callback = @(src, event) do_medfilt(imgobj, s.Value);         
...
b_save_nii = uicontrol('Parent', f, 'Style', 'pushbutton', 'Position', [320, 110, 30, 30], ...
          ...
          b_save_nii.Callback = @(src, event) display_save_nii_axial(nii, [floder 'updated']);
